...
test("path must be absolute (promise)", async ({ rejects }) => {
  ...
  await rejects(
    () => isFileEsm("./bar/foo.mjs"),
    Error(ERR_PATH_MUST_BE_ABSOLUTE)
  );
});
...
test("path must have ext (promise)", async ({ rejects }) => {
  await rejects(
    () => isFileEsm("/bar/foo"),
    Error(ERR_PATH_MUST_HAVE_VALID_EXT)
  );
});

test("path must have valid ext (promise)", async ({ rejects }) => {
  await rejects(
    () => isFileEsm("/bar/foo.ext"),
    Error(ERR_PATH_MUST_HAVE_VALID_EXT)
  );
});
...
test("path must exist (promise)", async ({ rejects }) => {
  await rejects(
    () => isFileEsm(join(fixtures, "bar", "foo.js")),
    Error(ERR_PATH_MUST_EXIST)
  );
});
...
test("package.json read error propagation (promise)", async ({
  rejects,
  tearDown,
}) => {
  const inputPath = join(fixtures, "type-bad-js", "file.js");
...
test("path must exist (promise)", async ({ throws }) => {
  throws(
    () => isFileEsm.sync(join(fixtures, "bar", "foo.js")),
    Error(ERR_PATH_MUST_EXIST)
  );
});
...
test("package.json read error propagation (sync)", async ({
  throws,
  tearDown,
}) => {
  const inputPath = join(fixtures, "type-bad-js", "file.js");
...
test("package.json read error propagation (callback)", async ({
  match,
  tearDown,
}) => {
  const inputPath = join(fixtures, "type-bad-js", "file.js");
