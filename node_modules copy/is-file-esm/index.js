...
const ERR_PATH_MUST_HAVE_VALID_EXT =
  "is-esm: path must be to a file with an extension of .js, .mjs or .cjs";

function isFileEsm(path, cb) {
  if (arguments.length <= 1) return isFileEsmPromise(path);
  if (typeof path !== "string")
    return void tick(() => cb(Error(ERR_PATH_MUST_BE_STRING)));
  if (isAbsolute(path) === false)
    return void tick(() => cb(Error(ERR_PATH_MUST_BE_ABSOLUTE)));
  const extMatch = /\.(c|m)?js/.exec(extname(path));
  if (extMatch === null)
    return void tick(() => cb(Error(ERR_PATH_MUST_HAVE_VALID_EXT)));
  access(path, F_OK, (err) => {
    ...
    readPkgUp({ cwd }, (err, pkg) => {
      ...
      switch (type) {
        ...
        default:
          return void cb(
            Object.assign(Error(`package.json type "${type}" not recognized`), {
              meta: { type, extType, path, pkgPath: pkg.path },
            })
          );
      }
    });
...
isFileEsm.sync = function isFileEsmSync(path) {
  ...
  switch (type) {
    case undefined: {
      if (extType === "j")
        return { esm: false, type, extType, path, pkgPath: pkg.path };
      if (extType === "m")
        return { esm: true, type, extType, path, pkgPath: pkg.path };
      /* istanbul ignore else */
      if (extType === "c")
        return { esm: false, type, extType, path, pkgPath: pkg.path };
      // unreachable
