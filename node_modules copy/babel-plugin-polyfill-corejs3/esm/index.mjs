...
const { types: t$2 } = _babel.default || _babel;
function canSkipPolyfill(desc, path) {
  const { node, parent } = path;
  switch (desc.name) {
    case "es.string.split": {
      if (
        !t$2.isCallExpression(parent, {
          callee: node,
        })
      )
        return false;
...
const { types: t$1 } = _babel.default || _babel;
...
function callMethod(path, id) {
  ...
  path.replaceWith(
    t$1.memberExpression(
      t$1.callExpression(id, [context2]),
      t$1.identifier("call")
    )
  );
...
var index = defineProvider(function (
  ...
  function maybeInjectPure(desc, hint, utils, object) {
    if (
      ...
      if (
        useBabelRuntime &&
        !(useProposalBase ? proposals : stable).has(desc.pure)
      ) {
        return;
      }
