...
function needsWrapping(node) {
  ...
  if (
    core.types.isCallExpression(node) ||
    core.types.isOptionalCallExpression(node) ||
    core.types.isNewExpression(node)
  ) {
  ...
  if (
    core.types.isMemberExpression(node) ||
    core.types.isOptionalMemberExpression(node)
  ) {
    return (
  ...
  if (
    core.types.isFunctionExpression(node) ||
    core.types.isArrowFunctionExpression(node) ||
    core.types.isClassExpression(node)
  ) {
    return false;
  }
...
function wrapInitializer(path) {
  ...
  if (value && needsWrapping(value)) {
    path.node.value = core.types.callExpression(
      core.types.arrowFunctionExpression([], value),
      []
    );
  }
}
