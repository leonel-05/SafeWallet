...
function wrapLoopBody(loopPath, captured, updatedBindingsUsages) {
  ...
  for (const [name, updatedUsage] of updatedBindingsUsages) {
    ...
    for (const path of updatedUsage)
      path.replaceWith(_core.types.identifier(innerName));
  }
  ...
  for (const varPath of state.vars) {
    ...
    if (assign.length > 0) {
      const replacement =
        assign.length === 1
          ? assign[0]
          : _core.types.sequenceExpression(assign);
  ...
  if (labelNum + returnNum === 0) {
    ...
    for (const path of state.breaksContinues) {
      ...
      path.replaceWith(
        _core.types.addComment(
          _core.types.returnStatement(_core.types.numericLiteral(1)),
          "trailing",
          " " + name,
          true
        )
    ...
    if (varPath.isVariableDeclaration()) {
      ...
      bodyStmts.push(
        _core.types.expressionStatement(
          _core.types.assignmentExpression(
            "=",
            _core.types.identifier(completionId),
            call
          )
        )
      );
    ...
    for (const path of state.breaksContinues) {
      ...
      path.replaceWith(
        _core.types.addComment(
          _core.types.returnStatement(_core.types.numericLiteral(i)),
          "trailing",
          " " + name,
          true
        )
      ...
      bodyStmts.push(_core.template.statement.ast`
        if (${_core.types.identifier(
          completionId
        )} === ${_core.types.numericLiteral(i)}) ${node}
      `);
    }
    if (returnNum) {
