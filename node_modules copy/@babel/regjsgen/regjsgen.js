...
(function () {
  ...
  function generateCharacterClass(node) {
    ...
    var kind = node.kind;
    var separator =
      kind === "intersection" ? "&&" : kind === "subtraction" ? "--" : "";
  ...
  function generateClassStrings(node) {
    ...
    return (
      "\\q{" + generateSequence(generateClassString, node.strings, "|") + "}"
    );
  }
    ...
    switch (node.behavior) {
      ...
      case "ignore":
        if (node.modifierFlags) {
          result += "?";
          if (node.modifierFlags.enabling)
            result += node.modifierFlags.enabling;
          if (node.modifierFlags.disabling)
            result += "-" + node.modifierFlags.disabling;
          result += ":";
        } else {
          result += "?:";
        }
