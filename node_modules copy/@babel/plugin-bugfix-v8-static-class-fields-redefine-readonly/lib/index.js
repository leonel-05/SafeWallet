...
function isStaticFieldWithValue(node) {
  return (
    (core.types.isClassProperty(node) ||
      core.types.isClassPrivateProperty(node)) &&
    node.static &&
    !!node.value
  );
}
...
function isReferenceOrThis(node, name) {
  return (
    core.types.isThisExpression(node) ||
    (name &&
...
function buildFieldsReplacement(fields, scope, file) {
  return core.types.staticBlock(
    fields.map((field) => {
      const key =
        field.computed || !core.types.isIdentifier(field.key)
          ? field.key
          : core.types.stringLiteral(field.key.name);
      return core.types.expressionStatement(
        core.types.callExpression(file.addHelper("defineProperty"), [
        ...
        for (let i = ranges.length - 1; i >= 0; i--) {
          ...
          startPath.replaceWith(
            buildFieldsReplacement(
              path.node.body.body.slice(start, end),
              path.scope,
              this.file
            )
          );
