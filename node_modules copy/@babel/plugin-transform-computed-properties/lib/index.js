  ...
  (api, options) => {
    ...
    function pushComputedPropsLoose(info) {
      ...
      for (const prop of computedProps) {
        if (
          _core.types.isObjectMethod(prop) &&
          ...
          if (computedProps.length === 1) {
            ...
            body.push(
              _core.types.expressionStatement(
                buildDefineAccessor(state, _core.types.cloneNode(objId), prop)
              )
            );
          }
        } else {
      ...
      for (const chunk of computedPropsChunks) {
        ...
        for (const prop of chunk) {
          if (
            ...
            node = _core.types.callExpression(
              state.addHelper("defineProperty"),
              [node, _core.types.toComputedKey(prop), getValue(prop)]
            );
          }
        }
