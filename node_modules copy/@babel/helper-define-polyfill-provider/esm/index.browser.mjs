...
const { types: t$1, template: template } = _babel.default || _babel;
          ...
          (isScript, source, name) => {
            ...
            return {
              node: isScript
                ...
                `)
                : t$1.importDeclaration(
                    [t$1.importSpecifier(id, name)],
                    source
                  ),
              name: id.name,
            };
...
class ImportsCachedInjector {
  ...
  _injectImport(programPath, node, moduleName) {
    ...
    const lastImports =
      (_this$_lastImports$ge = this._lastImports.get(programPath)) != null
        ? _this$_lastImports$ge
        : [];
    const isPathStillValid = (path) =>
      path.node &&
...
function isRemoved(path) {
  ...
  if (path.listKey) {
    ...
    if (
      !(
        (_path$parentPath$node = path.parentPath.node) != null &&
        (_path$parentPath$node = _path$parentPath$node[path.listKey]) != null &&
        _path$parentPath$node.includes(path.node)
      )
    )
      return true;
  } else {
...
function instantiateProvider(
  ...
  const getUtils = createUtilsGetter(
    new ImportsCachedInjector(
      (moduleName) => resolve(dirname, moduleName, absoluteImports),
      (name) => {
        var _polyfillsNames$get, _polyfillsNames;
        return (_polyfillsNames$get =
          (_polyfillsNames = polyfillsNames) == null
            ? void 0
            : _polyfillsNames.get(name)) != null
          ? _polyfillsNames$get
          : Infinity;
      }
    )
  ...
  const api = {
    ...
    debug(name) {
      ...
      (_debugLog$polyfillsSu = (_debugLog = debugLog()).polyfillsSupport) !=
      null
  ...
  if (methodName === "usageGlobal") {
    callProvider = (payload, path) => {
      ...
      return (_ref = provider[methodName](payload, utils, path)) != null
        ? _ref
        : false;
    };
  } else {
