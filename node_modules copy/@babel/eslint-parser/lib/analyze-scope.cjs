...
function getVisitorValues(nodeType, client) {
  ...
  )(VISITOR_KEYS).reduce((acc, [key, value]) => {
...
class Referencer extends OriginalReferencer {
  ...
  visitClass(node) {
    ...
    this._visitTypeAnnotation(
      (_node$superTypeParame = node.superTypeParameters) == null
        ? void 0
        : _node$superTypeParame.params
    );
  ...
  _visitTypeAnnotation(node) {
    ...
    const visitorValues = getVisitorValues(
      node.type,
      _classPrivateFieldGet(_client, this)
    );
    if (!visitorValues) {
      return;
    }
...
module.exports = function analyzeScope(ast, parserOptions, client) {
  ...
  const options = {
    ...
    nodejsScope:
      ...
      ((_parserOptions$ecmaFe = parserOptions.ecmaFeatures) == null
        ? void 0
        : _parserOptions$ecmaFe.globalReturn) === true,
    impliedStrict: false,
