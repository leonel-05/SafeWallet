    ...
    if (prop.isPrivate()) {
      ...
      if (!update) {
        ...
        if (
          ...
          (_classBrandId = classBrandId) != null
            ? _classBrandId
...
const privateInVisitor = privateNameVisitorFactory({
  BinaryExpression(path, { file }) {
    ...
    if (isStatic) {
      path.replaceWith(
        _core.template.expression.ast`${buildCheckInRHS(
          right,
          file
        )} === ${_core.types.cloneNode(this.classRef)}`
      );
      return;
    }
    path.replaceWith(
      _core.template.expression.ast`${_core.types.cloneNode(
        id
      )}.has(${buildCheckInRHS(right, file)})`
    );
  },
});
