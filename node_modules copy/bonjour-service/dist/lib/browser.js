...
class Browser extends events_1.EventEmitter {
  constructor(mdns, opts, onup) {
    ...
    this.txt = new dns_txt_1.default(
      opts !== null && opts.txt != null ? opts.txt : undefined
    );
  ...
  updateService(service) {
    var _a;
    if (
      (0, equal_txt_1.default)(
        service.txt,
        ((_a = this._services.find((s) =>
          (0, dns_equal_1.default)(s.fqdn, service.fqdn)
        )) === null || _a === void 0
          ? void 0
          : _a.txt) || {}
      )
    ...
    return packet.answers
      ...
      .filter(
        (rr) =>
          rr.type === "PTR" &&
          rr.ttl === 0 &&
          (0, dns_equal_1.default)(rr.name, name)
      )
      .map((rr) => rr.data);
  }
  buildServicesFor(name, packet, txt, referer) {
    var records = packet.answers
      .concat(packet.additionals)
      .filter((rr) => rr.ttl > 0);
    return records
      .filter(
        (rr) => rr.type === "PTR" && (0, dns_equal_1.default)(rr.name, name)
      )
      .map((ptr) => {
        const service = {
          addresses: [],
        ...
        records
          .filter((rr) => {
            return (
              ...
              (0, dns_equal_1.default)(rr.name, ptr.data)
            );
          })
          .forEach((rr) => {
            if (rr.type === "SRV") {
              ...
              var name = parts[0];
              var types = (0, service_types_1.toType)(
