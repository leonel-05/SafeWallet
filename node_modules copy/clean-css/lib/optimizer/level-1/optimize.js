...
function optimizeBody(rule, properties, context) {
  ...
  for (i = 0, l = _properties.length; i < l; i++) {
    ...
    name = property.name;
    propertyOptimizer =
      (configuration[name] && configuration[name].propertyOptimizer) || noop;
    valueOptimizers = (configuration[name] &&
      configuration[name].valueOptimizers) || [optimizers.whiteSpace];
    isVariable = VARIABLE_PROPERTY_NAME_PATTERN.test(name);

    if (isVariable) {
      valueOptimizers =
        options.variableOptimizers.length > 0
...
function buildVariableOptimizers(options) {
  return options.level[OptimizationLevel.One].variableValueOptimizers.map(
    function (optimizer) {
...
function level1Optimize(tokens, context) {
  ...
  options.variableOptimizers =
    options.variableOptimizers || buildVariableOptimizers(options);
