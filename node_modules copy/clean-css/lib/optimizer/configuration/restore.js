...
function background(property, configuration, lastInMultiplex) {
  ...
  for (var i = components.length - 1; i >= 0; i--) {
    ...
    if (component.name == "background-clip") {
      ...
      if (
        isDefault ||
        (configuration[component.name].multiplexLastOnly && !lastInMultiplex)
      ) {
        continue;
      }
  ...
  if (restored.length === 0) {
    restored.push([
      ...
      configuration[property.name].defaultValue,
    ]);
  }
...
function font(property, configuration) {
  ...
  if (
    ...
    configuration[components[componentIndex].name].defaultValue
  ) {
  ...
  for (var i = components.length - 1; i >= 0; i--) {
    ...
    if (
      ...
      ("keepUnlessDefault" in descriptor &&
        !isDefault(components, configuration, descriptor.keepUnlessDefault))
    ) {
...
function isDefault(components, configuration, propertyName) {
  ...
  for (i = 0, l = components.length; i < l; i++) {
    ...
    if (
      ...
      component.value[0][1] == configuration[propertyName].defaultValue
    ) {
      return true;
    }
  }
