...
function mergeIntoShorthands(properties, validator) {
  ...
  for (i = 0, l = properties.length; i < l; i++) {
    ...
    if (
      descriptor &&
      descriptor.singleTypeComponents &&
      !hasSameValues(property)
    ) {
      continue;
    }
...
function replaceWithInheritBestFit(
  ...
  var lastComponent =
    candidateComponents[Object.keys(candidateComponents).pop()];
...
function replaceWithShorthand(
  ...
  var all;
  var insertAt = inferInsertAtFrom(
    properties,
    candidateComponents,
    shorthandName
  );
...
function inferInsertAtFrom(properties, candidateComponents, shorthandName) {
  ...
  var firstCandidatePosition =
    candidateComponents[candidateComponentNames[0]].position;
  var lastCandidatePosition =
    candidateComponents[
      candidateComponentNames[candidateComponentNames.length - 1]
    ].position;

  if (
    shorthandName == "border" &&
    traversesVia(
      properties.slice(firstCandidatePosition, lastCandidatePosition),
      "border-image"
    )
  ) {
