...
function populateComponents(properties, validator, warnings) {
  ...
  for (var i = properties.length - 1; i >= 0; i--) {
    ...
    if (!property.dynamic && descriptor && descriptor.shorthand) {
      if (
        onlyValueIsVariable(property, validator) ||
        moreThanOneValueIsVariable(property, validator)
      ) {
      ...
      try {
        ...
        if (descriptor.shorthandComponents) {
          for (j = 0, m = property.components.length; j < m; j++) {
            ...
            component.components = configuration[component.name].breakUp(
              component,
              configuration,
              validator
            );
...
function onlyValueIsVariable(property, validator) {
  return (
    property.value.length == 1 && validator.isVariable(property.value[0][1])
  );
}
...
function moreThanOneValueIsVariable(property, validator) {
  return (
    ...
    property.value.filter(function (value) {
