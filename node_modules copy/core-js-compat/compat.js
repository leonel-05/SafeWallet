...
function getModules(filter) {
  if (typeof filter == "string") {
    ...
    return atLeastSomeModules(
      allModules.filter((it) => it.startsWith(filter)),
      filter
    );
  }
  if (filter instanceof RegExp)
    return atLeastSomeModules(
      allModules.filter((it) => filter.test(it)),
      filter
    );
...
function checkModule(name, targets) {
  ...
  for (const [engine, version] of targets) {
    if (
      !has(requirements, engine) ||
      compare(version, "<", requirements[engine])
    ) {
...
module.exports = function ({
  ...
  modules = intersection(
    modules,
    version ? getModulesListForTargetVersion(version) : allModules
  );
