...
function defineComponent(options, extraOptions) {
  return shared.isFunction(options)
    ? // #8236: extend call and options.name access are considered side-effects
      ...
      /* @__PURE__ */ (() =>
        shared.extend({ name: options.name }, extraOptions, {
          setup: options,
        }))()
    : options;
}
      ...
      if (props) {
        {
          ...
          for (const key in props) {
            ...
            if (
              ...
              (shared.isOn(key) && !shared.isReservedProp(key)) || // force hydrate v-bind with .prop modifiers
              key[0] === "." ||
              isCustomElement
            ) {
...
function propHasMismatch(el, key, clientValue, vnode, instance) {
  ...
  if (key === "class") {
    ...
    expected = shared.isString(clientValue)
      ? clientValue
      : shared.stringifyStyle(shared.normalizeStyle(clientValue));
    ...
    (el instanceof SVGElement && shared.isKnownSvgAttr(key)) ||
    (el instanceof HTMLElement &&
      (shared.isBooleanAttr(key) || shared.isKnownHtmlAttr(key)))
  ) {
    if (shared.isBooleanAttr(key)) {
      ...
      expected = shared.isRenderableAttrValue(clientValue)
        ? String(clientValue)
        : false;
    }
    if (actual !== expected) {
...
function renderList(source, renderItem, cache, index) {
  ...
  if (sourceIsArray || shared.isString(source)) {
    const sourceIsReactiveArray =
      sourceIsArray && reactivity.isReactive(source);
    let needsWrap = false;
...
const hasSetupBinding = (state, key) =>
  state !== shared.EMPTY_OBJ &&
  !state.__isScriptSetup &&
  shared.hasOwn(state, key);
const PublicInstanceProxyHandlers = {
  get({ _: instance }, key) {
    ...
    if (publicGetter) {
      ...
      (!shared.isString(key) || // #1091 avoid internal isRef/isVNode checks on component instance leading
...
const RuntimeCompiledPublicInstanceProxyHandlers =
  /* @__PURE__ */ shared.extend({}, PublicInstanceProxyHandlers, {
    get(target, key) {
...
function mergeObjectOptions(to, from) {
  return to
    ? shared.extend(/* @__PURE__ */ Object.create(null), to, from)
    : from;
}
    ...
    for (const key in raw) {
      ...
      if (validatePropName(normalizedKey)) {
        ...
        const prop = (normalized[normalizedKey] =
          shared.isArray(opt) || shared.isFunction(opt)
            ? { type: opt }
            : shared.extend({}, opt));
        ...
        if (shared.isArray(propType)) {
          ...
          shouldCast =
            shared.isFunction(propType) && propType.name === "Boolean";
        }
  ...
  const res = reactivity.customRef((track, trigger) => {
    ...
    return {
      ...
      set(value) {
        ...
        if (
          !shared.hasChanged(emittedValue, localValue) &&
          !(
            prevSetValue !== shared.EMPTY_OBJ &&
            shared.hasChanged(value, prevSetValue)
          )
        ) {
          return;
        }
        ...
        if (
          shared.hasChanged(value, emittedValue) &&
          shared.hasChanged(value, prevSetValue) &&
          !shared.hasChanged(emittedValue, prevEmittedValue)
        ) {
          trigger();
        }
        prevSetValue = value;
...
const getModelModifiers = (props, modelName) => {
  return modelName === "modelValue" || modelName === "model-value"
    ...
    : props[`${modelName}Modifiers`] ||
        props[`${shared.camelize(modelName)}Modifiers`] ||
        props[`${shared.hyphenate(modelName)}Modifiers`];
};
...
function emit(instance, event, ...rawArgs) {
  ...
  const props = instance.vnode.props || shared.EMPTY_OBJ;
  {
    ...
    if (emitsOptions) {
      if (!(event in emitsOptions) && true) {
        if (
          !propsOptions ||
          !(shared.toHandlerKey(shared.camelize(event)) in propsOptions)
        ) {
          warn$1(
            `Component emitted event "${event}" but it is neither declared in the emits option nor as an "${shared.toHandlerKey(
              shared.camelize(event)
            )}" prop.`
          );
        }
      } else {
  ...
  let handlerName;
  let handler =
    props[(handlerName = shared.toHandlerKey(event))] || // also try camelCase event handler (#2249)
...
function guardReactiveProps(props) {
  ...
  return reactivity.isProxy(props) || isInternalObject(props)
    ? shared.extend({}, props)
    : props;
}
function cloneVNode(
  ...
  const cloned = {
    ...
    children:
      patchFlag === -1 && shared.isArray(children)
  ...
  if (instance.exposed) {
    return (
      ...
      (instance.exposeProxy = new Proxy(
        reactivity.proxyRefs(reactivity.markRaw(instance.exposed)),
        {
          get(target, key) {
            if (key in target) {
              return target[key];
