...
const parserOptions = {
  ...
  isNativeTag: (tag) =>
    shared.isHTMLTag(tag) || shared.isSVGTag(tag) || shared.isMathMLTag(tag),
...
const isEventOptionModifier =
  /* @__PURE__ */ shared.makeMap(`passive,once,capture`);
...
const resolveModifiers = (key, modifiers, context, loc) => {
  ...
  for (let i = 0; i < modifiers.length; i++) {
    ...
    if (
      modifier === "native" &&
      compilerCore.checkCompatEnabled("COMPILER_V_ON_NATIVE", context, loc)
    ) {
...
const stringifyStatic = (children, context, parent) => {
  ...
  const isParentCached =
    ...
    parent.codegenNode.children.type === 20;
  let nc = 0;
  let ec = 0;
  const currentChunk = [];
...
const getCachedNode = (node) => {
  if (
    ((node.type === 1 && node.tagType === 0) || node.type === 12) &&
    node.codegenNode &&
    node.codegenNode.type === 20
  ) {
    ...
    for (let i = 0; i < node2.props.length; i++) {
      ...
      if (p.type === 7 && p.name === "bind") {
        if (
          p.arg &&
          (p.arg.type === 8 ||
            (p.arg.isStatic && !isStringifiableAttr(p.arg.content, node2.ns)))
        ) {
          return bail();
        }
        ...
        if (
          isOptionTag &&
          compilerCore.isStaticArgOf(p.arg, "value") &&
          p.exp &&
          p.exp.ast &&
