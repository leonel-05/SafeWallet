  ...
  startEntity() {
    {
      ...
      this.entityDecoder.startEntity(
        this.baseState === 1 || this.baseState === 32
          ? decode_js.DecodingMode.Legacy
          : decode_js.DecodingMode.Attribute
      );
    }
  }
  stateInEntity() {
  ...
  estreeWalker.walk(root, {
    enter(node, parent) {
      ...
      if (node.type === "Identifier") {
        ...
        node.type === "ObjectProperty" && // eslint-disable-next-line no-restricted-syntax
...
const isMemberExpressionNode = (exp, context) => {
  try {
    let ret =
      exp.ast ||
      parser.parseExpression(getExpSource(exp), {
  ...
  switch (node.type) {
    ...
    case 8:
      return node.children.some(
        (c) => shared.isObject(c) && hasScopeRef(c, ids)
      );
    case 5:
    case 12:
...
function walk(node, parent, context, doNotHoistNode = false, inFor = false) {
  ...
  if (toCache.length === children.length && node.type === 1) {
    if (
      ...
      !shared.isArray(parent.codegenNode.children) &&
...
function genNodeListAsArray(nodes, context) {
  const multilines =
    nodes.length > 3 || nodes.some((n) => shared.isArray(n) || !isText(n));
  context.push(`[`);
...
const transformOn = (dir, node, context, augmentor) => {
  ...
  if (exp) {
    ...
    if (context.prefixIdentifiers) {
      ...
      shouldCache =
        context.cacheHandlers && // unnecessary to cache inside v-once
...
function baseCompile(source, options = {}) {
  ...
  const ast = shared.isString(source)
