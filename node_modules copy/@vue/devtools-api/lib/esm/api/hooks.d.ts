...
export interface HookPayloads {
  ...
  [Hooks.EDIT_COMPONENT_STATE]: {
    ...
    set: (
      ...
      path?: string | string[],
      value?: any,
      cb?: (object: any, field: string, value: any) => void
    ) => void;
  };
...
export interface Hookable<TContext> {
  ...
  visitComponentTree: (
    handler: HookHandler<HookPayloads[Hooks.VISIT_COMPONENT_TREE], TContext>
  ) => any;
  ...
  getComponentInstances: (
    handler: HookHandler<HookPayloads[Hooks.GET_COMPONENT_INSTANCES], TContext>
  ) => any;
  getElementComponent: (
  ...
  getComponentRootElements: (
    handler: HookHandler<
      HookPayloads[Hooks.GET_COMPONENT_ROOT_ELEMENTS],
      TContext
    >
  ) => any;
  editComponentState: (
    handler: HookHandler<HookPayloads[Hooks.EDIT_COMPONENT_STATE], TContext>
  ) => any;
  getComponentDevtoolsOptions: (
    handler: HookHandler<
      HookPayloads[Hooks.GET_COMPONENT_DEVTOOLS_OPTIONS],
      TContext
    >
  ) => any;
  getComponentRenderCode: (
    handler: HookHandler<
      HookPayloads[Hooks.GET_COMPONENT_RENDER_CODE],
      TContext
    >
  ) => any;
  inspectTimelineEvent: (
    handler: HookHandler<HookPayloads[Hooks.INSPECT_TIMELINE_EVENT], TContext>
  ) => any;
  timelineCleared: (
    handler: HookHandler<HookPayloads[Hooks.TIMELINE_CLEARED], TContext>
  ) => any;
  getInspectorTree: (
  ...
  editInspectorState: (
    handler: HookHandler<HookPayloads[Hooks.EDIT_INSPECTOR_STATE], TContext>
  ) => any;
  setPluginSettings: (
    handler: HookHandler<HookPayloads[Hooks.SET_PLUGIN_SETTINGS], TContext>
  ) => any;
}
