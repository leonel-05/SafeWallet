...
function getEslintConfigName(eslint) {
  ...
  if (eslint.extends instanceof Array) {
    config = eslint.extends.find((configName) =>
      configName.startsWith("plugin:vue/")
    );
  }
...
function getDefaultValue(rule, data) {
  ...
  return CATEGORIES.indexOf(ruleCategory) <=
    CATEGORIES.indexOf(currentCategory.split("/")[1])
    ? RULE_SETTING_ERROR
    : RULE_SETTING_OFF;
}

function getEslintPrompts(data, rules) {
  const allRules = Object.keys(rules).map((ruleKey) => ({
    ...rules[ruleKey],
    name: `vue/${ruleKey}`,
  }));
  ...
  return CATEGORIES.map((category) =>
    allRules.filter(
      (rule) =>
        ...
        (category === "uncategorized" && rule.meta.docs.category === undefined)
    )
  )
    .reduce((acc, rulesArr) => [...acc, ...rulesArr], [])
    .map((rule) => {
      const value =
        data.eslint && data.eslint.rules && data.eslint.rules[rule.name];

      return {
        name: rule.name,
        type: "list",
        message: rule.name,
        group: `org.vue.eslint.config.eslint.groups.${
          rule.meta.docs.category || "uncategorized"
        }`,
        ...
        choices:
          !value || RULE_SETTINGS.indexOf(value) > -1
            ? defaultChoices
            : [
                ...defaultChoices,
                {
        ...
        prompts: [
          {
            ...
            description:
              "org.vue.eslint.config.eslint.general.lintOnSave.description",
          ...
          },
          {
            ...
            description:
              "org.vue.eslint.config.eslint.general.config.description",
            link: "https://github.com/vuejs/eslint-plugin-vue",
            default: `plugin:vue/${DEFAULT_CATEGORY}`,
            choices: CATEGORIES.filter(
  ...
  for (const prompt of prompts) {
    ...
    if (prompt.id === "config") {
      if (eslintData.extends instanceof Array) {
        const vueEslintConfig = eslintData.extends.find(
          (config) => config.indexOf("plugin:vue/") === 0
        );
      ...
      eslintData[`rules.${prompt.id}`] = await api.getAnswer(
        prompt.id,
        JSON.parse
      );
    } else {
