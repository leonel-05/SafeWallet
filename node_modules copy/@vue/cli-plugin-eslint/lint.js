...
module.exports = async function lint(args = {}, api) {
  ...
  if (
    !fs.existsSync(api.resolve(".eslintignore")) &&
    ...
    config.overrideConfig.ignorePatterns = ["!.*.js", "!{src,tests}/**/.*.js"];
  }
  ...
  const reportWarningCount = resultResults.reduce(
  ...
  if (config.outputFile) {
    ...
    try {
      ...
      log(
        `Error saving lint results to ${chalk.blue(
          outputFilePath
        )}: ${chalk.red(err)}`
      );
    }
...
function normalizeConfig(args) {
  ...
  for (const key in args) {
    if (renamedArrayArgs[key]) {
      ...
      const obj = arrayToBoolObject(
        args[key].split(","),
        renamedObjectArgs[key].def
      );
