...
${filename}
${(0, import_code_frame.codeFrameColumns)(file.code, {
  start: {
    ...
    column: node.loc.end.column + 1,
      ...
      post(file) {
        for (const helper of helpers) {
          (0, import_helper_module_imports.addNamed)(
            file.path,
            `_${helper}`,
            "vue"
          );
    ...
    function inferComponentName(path) {
      ...
      if (!id || !id.isIdentifier() || !init || !init.isCallExpression())
        return;
      if (
        !((_a = init.get("callee")) == null
          ? void 0
          : _a.isIdentifier({ name: "defineComponent" }))
      )
        return;
    ...
    function processEmits(comp, options2) {
      ...
      if (
        ...
        return;
      const emitType =
        (_a = setupCtx.typeParameters) == null ? void 0 : _a.params[0];
      if (!emitType) return;
...
function checkDefineComponent(path) {
  ...
  const defineCompImport =
    (_a = path.scope.getBinding("defineComponent")) == null
      ? void 0
      : _a.path.parent;
