...
function processIfBranch(branch, context, disableNestedFragments = false) {
  ...
  const needFragmentWrapper =
    !disableNestedFragments &&
    (children.length !== 1 || children[0].type !== 1) && // optimize away nested fragments when the only child is a ForNode
...
function ssrProcessFor(node, context, disableNestedFragments = false) {
  const needFragmentWrapper =
    !disableNestedFragments &&
    (node.children.length !== 1 || node.children[0].type !== 1);
...
const ssrTransformSlotOutlet = (node, context) => {
  if (compilerDom.isSlotOutlet(node)) {
    const { slotName, slotProps } = compilerDom.processSlotOutlet(
      node,
      context
    );
    const args = [
    ...
    if (parent) {
      ...
      if (
        parent.type === 1 &&
        parent.tagType === 1 &&
        ((componentType = compilerDom.resolveComponentType(
          parent,
          context,
          true
        )) === compilerDom.TRANSITION ||
          componentType === compilerDom.TRANSITION_GROUP) &&
        children.filter((c) => c.type === 1).length === 1
      ) {
...
function buildSSRProps(props, directives, context) {
  ...
  if (directives.length) {
    for (const dir of directives) {
      mergePropsArgs.push(
        compilerDom.createCallExpression(
          context.helper(SSR_GET_DIRECTIVE_PROPS),
          [`_ctx`, ...compilerDom.buildDirectiveArgs(dir, context).elements]
        )
      );
    }
  }
...
function ssrTransformTransitionGroup(node, context) {
  return () => {
    ...
    if (tag) {
      ...
      let propsExp = null;
      if (props || directives.length) {
        propsExp = compilerDom.createCallExpression(
          context.helper(SSR_RENDER_ATTRS),
...
const ssrTransformComponent = (node, context) => {
  ...
  const isDynamicComponent =
    shared.isObject(component) &&
    component.callee === compilerDom.RESOLVE_DYNAMIC_COMPONENT;
...
function ssrCodegenTransform(ast, options) {
  ...
  if (options.ssrCssVars) {
    const cssContext = compilerDom.createTransformContext(
...
function compile(source, options = {}) {
  ...
  const ast =
    typeof source === "string"
      ? compilerDom.baseParse(source, options)
      : source;
