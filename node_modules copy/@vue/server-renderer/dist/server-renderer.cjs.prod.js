...
function ssrRenderDynamicAttr(key, value, tag) {
  ...
  const attrKey =
    tag && (tag.indexOf("-") > 0 || shared.isSVGTag(tag))
      ? key
...
function ssrCompile(template, instance) {
  ...
  finalCompilerOptions.isNativeTag =
    finalCompilerOptions.isNativeTag || shared.NO;
...
function renderComponentSubTree(instance, slotScopeId) {
  ...
  if (shared.isFunction(comp)) {
    ...
    if (ssrRender) {
      ...
      Vue.warn(
        `Component ${componentName} is missing template or render function.`
      );
      push(`<!---->`);
    }
  }
