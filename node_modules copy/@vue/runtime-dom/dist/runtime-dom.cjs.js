...
function patchAttr(
  ...
  isSVG,
  instance,
  isBoolean = shared.isSpecialBooleanAttr(key)
) {
...
function parseName(name) {
  ...
  const event =
    name[2] === ":" ? name.slice(3) : shared.hyphenate(name.slice(2));
...
class VueElement extends BaseClass {
  ...
  _resolveProps(def) {
    const { props } = def;
    const declaredPropKeys = shared.isArray(props)
      ? props
      : Object.keys(props || {});
  ...
  _createVNode() {
    ...
    const vnode = runtimeCore.createVNode(
      this._def,
      shared.extend(baseProps, this._props)
    );
    if (!this._instance) {
      vnode.ce = (instance) => {
        ...
        const dispatch = (event, args) => {
          this.dispatchEvent(
            new CustomEvent(
              event,
              shared.isPlainObject(args[0])
                ? shared.extend({ detail: args }, args[0])
                : { detail: args }
            )
          );
        };
