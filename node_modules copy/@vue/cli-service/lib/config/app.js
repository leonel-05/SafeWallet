  ...
  api.chainWebpack((webpackConfig) => {
    ...
    if (!multiPageConfig) {
      ...
      publicCopyIgnore.push(
        api.resolve(htmlOptions.template).replace(/\\/g, "/")
      );
      ...
      pages.forEach((name) => {
        ...
        const templateWithoutLoader = template
          .replace(/^.+!/, "")
          .replace(/\?.+$/, "");

        // resolve page index template
        const hasDedicatedTemplate = fs.existsSync(
          api.resolve(templateWithoutLoader)
        );
        ...
        publicCopyIgnore.push(
          api.resolve(templateWithoutLoader).replace(/\\/g, "/")
        );
