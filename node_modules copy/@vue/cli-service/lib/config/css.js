...
module.exports = (api, rootOptions) => {
  api.chainWebpack((webpackConfig) => {
    ...
    const cssPublicPath =
      isAbsoluteUrl(rootOptions.publicPath) ||
    ...
    if (!hasPostCSSConfig) {
      ...
      const autoprefixerDirectory = path.dirname(
        require.resolve("autoprefixer/package.json")
      );
      const postcssPkg = loadModule(
        "postcss/package.json",
        autoprefixerDirectory
      );
      const postcssVersion = postcssPkg.version;
      if (!semver.satisfies(postcssVersion, "8.x")) {
        throw new Error(
          `The package manager has hoisted a wrong version of ${chalk.cyan(
            "postcss"
