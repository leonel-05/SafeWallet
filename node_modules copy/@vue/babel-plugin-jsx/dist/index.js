...
var import_babel_plugin_resolve_type = __toESM(
  require("@vue/babel-plugin-resolve-type")
);
...
var shouldTransformedToSlots = (tag) =>
...
var checkIsComponent = (path, state) => {
  ...
  return (
    !((_b = (_a = state.opts).isCustomElement) == null
      ? void 0
      : _b.call(_a, tag)) &&
    shouldTransformedToSlots(tag) &&
    !import_html_tags.default.includes(tag) &&
    !import_svg_tags.default.includes(tag)
  );
};
...
var getTag = (path, state) => {
  ...
  if (namePath.isJSXIdentifier()) {
    ...
    if (
      !import_html_tags.default.includes(name) &&
      !import_svg_tags.default.includes(name)
    ) {
...
var transformJSXSpreadChild = (path) =>
  t.spreadElement(path.get("expression").node);
...
var transformJSXSpreadAttribute = (nodePath, path, mergeProps, args) => {
  ...
  if (!properties) {
    if (argument.isIdentifier()) {
      walksScope(nodePath, argument.node.name, slotFlags_default.DYNAMIC);
    }
...
var parseModifiers = (value) =>
  t2.isArrayExpression(value)
    ? value.elements
        .map((el) => (t2.isStringLiteral(el) ? el.value : ""))
        .filter(Boolean)
    : [];
var parseDirectives = (params) => {
  ...
  if (t2.isArrayExpression(value)) {
    ...
    elementsList.forEach((element) => {
      ...
      if (
        second &&
        !t2.isArrayExpression(second) &&
        !t2.isSpreadElement(second)
      ) {
        args.push(second);
  ...
  return {
    ...
    directive: shouldResolve
      ? [
          ...
          vals[0] || value,
          ((_a = modifiersSet[0]) == null ? void 0 : _a.size)
            ? args[0] || t2.unaryExpression("void", t2.numericLiteral(0), true)
            : args[0],
          !!((_b = modifiersSet[0]) == null ? void 0 : _b.size) &&
            t2.objectExpression(
              [...modifiersSet[0]].map((modifier) =>
                t2.objectProperty(
                  t2.identifier(modifier),
                  t2.booleanLiteral(true)
                )
              )
            ),
        ].filter(Boolean)
  ...
  props.forEach((prop) => {
    if (prop.isJSXAttribute()) {
      ...
      if (!isConstant(attributeValue) || name === "ref") {
        if (
          !isComponent &&
          isOn(name) && // omit the flag for click handlers becaues hydration gives click
          // dedicated fast path.
      ...
      if (state.opts.transformOn && (name === "on" || name === "nativeOn")) {
        if (!state.get("transformOn")) {
          state.set(
            "transformOn",
            (0, import_helper_module_imports.addDefault)(
              path,
      ...
      if (isDirective(name)) {
        ...
        if (["models", "model"].includes(directiveName)) {
          values.forEach((value, index) => {
            ...
            const isDynamic =
              propName &&
              !t3.isStringLiteral(propName) &&
              !t3.isNullLiteral(propName);
            if (!directive) {
              properties.push(
                t3.objectProperty(
                  t3.isNullLiteral(propName)
                    ? t3.stringLiteral("modelValue")
                    : propName,
                  value,
                  isDynamic
                )
                ...
                properties.push(
                  t3.objectProperty(
                    isDynamic
                      ...
                      : t3.stringLiteral(
                          `${
                            (propName == null ? void 0 : propName.value) ||
                            "model"
                          }Modifiers`
                        ),
                    t3.objectExpression(
            ...
            properties.push(
              t3.objectProperty(
                ...
                t3.arrowFunctionExpression(
                  [t3.identifier("$event")],
                  t3.assignmentExpression("=", value, t3.identifier("$event"))
                ),
                isDynamic
              )
            );
...
var getChildren = (paths, state) =>
  paths
    ...
    .filter((value) => value != null && !t3.isJSXEmptyExpression(value));
  ...
  if (children.length > 1 || slots) {
      ...
      ? children.length
        ? t3.objectExpression(
            [
              ...
              ...(slots
                ? t3.isObjectExpression(slots)
                  ? slots.properties
                  : [t3.spreadElement(slots)]
                : []),
              optimize &&
                t3.objectProperty(
                  t3.identifier("_"),
    ...
    const objectExpression4 = t3.objectExpression(
      [
        ...
        optimize &&
          t3.objectProperty(t3.identifier("_"), t3.numericLiteral(slotFlag)),
      ].filter(Boolean)
    );
    if (t3.isIdentifier(child) && isComponent) {
      ...
      t3.isFunctionExpression(child) ||
      t3.isArrowFunctionExpression(child)
    ) {
  ...
  JSXFragment: {
    enter(path, state) {
      ...
      path.replaceWith(
        transformFragment(
          path,
          t4.isIdentifier(fragmentCallee)
            ? t4.jsxIdentifier(fragmentCallee.name)
            : t4.jsxMemberExpression(
...
var syntaxJsx = /* @__PURE__ */ interopDefault(
...
var src_default = (0, import_helper_plugin_utils.declare)(
  (api, opt, dirname) => {
    ...
    if (opt.resolveType) {
      ...
      resolveType = (0, import_babel_plugin_resolve_type.default)(
        api,
        opt.resolveType,
        dirname
      );
    ...
    return __spreadProps(__spreadValues({}, resolveType || {}), {
      ...
      visitor: __spreadProps(
        __spreadValues(
          __spreadValues(
            __spreadValues(
              {},
              resolveType == null ? void 0 : resolveType.visitor
            ),
              ...
              if (hasJSX(path)) {
                ...
                if ((0, import_helper_module_imports2.isModule)(path)) {
                  ...
                  importNames.forEach((name) => {
                    state.set(name, () => {
                      ...
                      const identifier5 = (0,
                      import_helper_module_imports2.addNamed)(
                        path,
                        name,
                        "vue",
                        {
                  ...
                  importNames.forEach((name) => {
                    state.set(name, () => {
                      if (!sourceName) {
                        sourceName = (0,
                        import_helper_module_imports2.addNamespace)(
                          path,
                          "vue",
                          {
                            ensureLiveReference: true,
                ...
                if (file.ast.comments) {
                  for (const comment of file.ast.comments) {
                    ...
                    if (jsxMatches) {
                      state.set("createVNode", () =>
                        t5.identifier(jsxMatches[1])
                      );
                    }
                  }
                }
